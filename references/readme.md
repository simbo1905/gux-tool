...
| `llm-vision` | Send zone crop images to a vision LLM for analysis. **Opt-in only** — requires API key. Not included in `all`. |
| `all` | Run every technique (except `ocr`, `llm-vision`, and `verify`). Combine into a single report. |
